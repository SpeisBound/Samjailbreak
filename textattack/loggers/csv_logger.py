...
class CSVLogger(Logger):
    ...
    def flush(self):
        if self.header:
            self.df.to_csv(self.filename, quoting=csv.QUOTE_NONNUMERIC, index=False, mode='a', header=self.header)
            self.header = False
        else:
            self.df.to_csv(self.filename, quoting=csv.QUOTE_NONNUMERIC, index=False, mode='a', header=self.header)
        self.df = pd.DataFrame()
    ...
    def flush_append(self, header=False):
        self.df.to_csv(self.filename, quoting=csv.QUOTE_NONNUMERIC, index=False, mode='a', header=header)
        self.df = pd.DataFrame()
        self._flushed = True
