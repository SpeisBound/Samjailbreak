...
class Trainer:
    ...
    def __init__(
        ...
        if train_dataset:
            assert isinstance(
                train_dataset, textattack.datasets_text_attack.Dataset
            ), f"`train_dataset` must be of type `textattack.datasets.Dataset`, but got type `{type(train_dataset)}`."

        if eval_dataset:
            assert isinstance(
                eval_dataset, textattack.datasets_text_attack.Dataset
    ...
    def _generate_adversarial_examples(self, epoch):
        ...
        adversarial_dataset = textattack.datasets_text_attack.Dataset(
            adversarial_examples,
    ...
    def train(self):
        ...
        return self.eval_on_generate_adv_examples(adv_dataset=adv_dataset)
    ...
    def eval_on_generate_adv_examples(self, adv_dataset):
        model = self.model_wrapper.model
        tokenizer = self.model_wrapper.tokenizer
        ...
        eval_dataloader = self.get_eval_dataloader(adv_dataset, eval_batch_size)
        ...
        with torch.no_grad():
            for step, batch in enumerate(eval_dataloader):
                ...
                all_targets.append(targets)
                rst_str = f'==============batch-{i}============\n preds:{preds}\n targets:{targets}\n'
                rst_details.append(rst_str)
                i = i+1

        preds = torch.cat(all_preds)
        targets = torch.cat(all_targets)
        logger.info(''.join(rst_details))
        ...
        if self._metric_name == "accuracy":
            logger.info(f"Model accuracy under {len(targets)} adv_examples: 0.00% \nFinal Model Eval {self._metric_name}: {eval_score * 100:.2f}%")
        else:
            logger.info(f"Final Model Eval {self._metric_name}: {eval_score:.4f}%")
    ...
    def _write_readme(self, best_eval_score, best_eval_score_epoch, train_batch_size):
        ...
        if isinstance(
                self.train_dataset, textattack.datasets_text_attack.HuggingFaceDataset
            ...
            if hasattr(self.train_dataset, "_subset"):
                ...
                self.eval_dataset, textattack.datasets_text_attack.HuggingFaceDataset
