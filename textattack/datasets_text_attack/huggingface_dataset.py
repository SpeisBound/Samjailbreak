...
#
# import textattack
# from textattack.datasets_text_attack import TextAttackDataset
#
...
from utils import DATASET_SAVE_PATH
...
class HuggingFaceDataset(Dataset):
    ...
    def __init__(
        ...
        if isinstance(name_or_dataset, datasets.Dataset):
            ...
            if subset:
                dataset_path = os.path.join(DATASET_SAVE_PATH,
                                            name_or_dataset, subset)
            else:
                dataset_path = os.path.join(DATASET_SAVE_PATH,
                                            name_or_dataset)

            self._dataset = datasets.load_from_disk(dataset_path)[split]
            subset_print_str = f", subset {_cb(subset)}" if subset else ""
            textattack.shared.logger.info(
