...
class MHASearch(SearchMethod):
    ...
    def __init__(self,
        ...
        self.init_adv_examples = init_adv_examples
    ...
    def _perform_search(self, initial_result):
        ...
        if self.init_adv_examples:
            ...
            init_adv_result, goal = self.get_goal_results([initial_adv_text])
        ...
        new_text, results, search_over = self._search_space_reduction(initial_result, init_adv_result[0])
        ...
        first_hit_spend_queries = 0
        hit_number = 0
        all_budgets = 15
        budgets_position = round(all_budgets/len(changed_indices))
        # word-position-isolated feedback
        print('changed_indices length:'+str(len(changed_indices)))
        sim_statisfy = 0
        adv_statisfy = 0
        ...
        for i in range(len(changed_indices)):
            results_texts = self.text_transformation(current_text=current_text, original_text=initial_text,
                                                     indices_to_modify=[changed_indices[i]])
            sims = self._get_similarity_score(transformed_texts=[rt.text for rt in results_texts],
                                              initial_text=initial_text.text).tolist()
            if len(results_texts) >= budgets_position:
                indexed_values = list(enumerate(sims))
                sorted_values = sorted(indexed_values, key=lambda x: x[1], reverse=True)
                top_indices = [item[0] for item in sorted_values[:budgets_position]]
                selected_texts = []
                for j in top_indices:
                    selected_texts.append(results_texts[j])
                results_texts = selected_texts

            results, search_over = self.get_goal_results(results_texts)
            position_hit_number = 0
            k = 0
            for result in results:
                total_query_number = total_query_number + 1
                if sims[k] > sim_current_text:
                    sim_statisfy = 1
                if result.goal_status == GoalFunctionResultStatus.SUCCEEDED:
                    adv_statisfy = 1
                    if sims[k] > sim_current_text:
                        position_hit_number = position_hit_number+1
                        if first_hit_spend_queries == 0:
                            first_hit_spend_queries = total_query_number
                k = k+1
            hit_number = hit_number + position_hit_number
            print('position_hit_number:' + str(position_hit_number))

        hit_rate_list = hit_number/total_query_number
        if first_hit_spend_queries == 0:
            first_hit_spend_queries = total_query_number
        first_hit_rate = first_hit_spend_queries/total_query_number

        MHASearch.hit_rate_list.append(hit_rate_list)
        MHASearch.first_hit_rate.append(first_hit_rate)
        MHASearch.adv_sfy.append(adv_statisfy)
        MHASearch.sim_sfy.append(sim_statisfy)

        print('total_query_number:'+str(total_query_number))
        print('hit_rate_list:')
        print(MHASearch.hit_rate_list)
        print('first_hit_rate:')
        print(MHASearch.first_hit_rate)
        print('adv_sfy:')
        print(MHASearch.adv_sfy)
        print('sim_sfy:')
        print(MHASearch.sim_sfy)
        # print('average_hit_rate_list:' + str(sum(MHASearch.hit_rate_list)/len(MHASearch.hit_rate_list)))
        # print('average_first_hit_rate:' + str(sum(MHASearch.first_hit_rate) / len(MHASearch.first_hit_rate)))
        ...
        # print('changed_indices length:'+str(len(changed_indices)))
        #
        # for i in range(int(all_budgets)):
        #     selected_index = random.randint(0, len(changed_indices) - 1)
        #     results_text = self.text_transformation(current_text=current_text, original_text=initial_text, indices_to_modify=[changed_indices[selected_index]])[0]
        #
        #     results, search_over = self.get_goal_results([results_text])
        #     total_query_number = total_query_number+1
        #     sims = self._get_similarity_score(transformed_texts=[results_text.text],
        #                                       initial_text=initial_text.text).tolist()
        #     for result in results:
        #         if result.goal_status == GoalFunctionResultStatus.SUCCEEDED:
        #             if sims[0] > sim_current_text:
        #                 hit_number = hit_number + 1
        #                 if first_hit_spend_queries == 0:
        #                     first_hit_spend_queries = total_query_number
        #             current_text = results_text
        #
        # hit_rate_list = hit_number/all_budgets
        # first_hit_rate = first_hit_spend_queries/all_budgets
        #
        # MHASearch.hit_rate_list.append(hit_rate_list)
        # MHASearch.first_hit_rate.append(1.0-first_hit_rate)
        #
        # print('total_query_number:'+str(total_query_number))
        # print('hit_rate_list:')
        # print(MHASearch.hit_rate_list)
