...
class GreedyWordSwapWIR(SearchMethod):
    ...
    def _get_index_order(self, initial_text):
        ...
        if self.wir_method == "unk":
        ...
        elif self.wir_method == "jacob_matrix":
            victim_model = self.get_model()
            index_scores = np.zeros(initial_text.num_words)
            grad_output = victim_model.get_grad(initial_text.tokenizer_input)
            gradient = grad_output["gradient"]  # numpy array id x gradient
            dist = []
            for i in range(len(gradient)):
                dist.append(0.0)
                for j in range(len(gradient[i])):
                    dist[i] += gradient[i][j] * gradient[i][j]
            ...
            for i, word in enumerate(initial_text.words):
                # matched_tokens = word2token_mapping[word]
                matched_tokens = word2token_mapping.get(word,None)
                if not matched_tokens:
                    index_scores[i] = 0.0
                else:
                    agg_grad = np.mean(gradient[matched_tokens], axis=0) # axis=0计算每一列的均值
