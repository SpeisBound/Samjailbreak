...
class HardLabelAttack(PopulationBasedSearch):
    ...
    def _perturb(
        ...
        print(f'new_text:{perturbed_text.text}')
    ...
    def _search_space_reduction(self, initial_result, initial_adv_text):
        ...
        for idx in different_indices:
            ...
            print(f'new_text:{new_text.text}')
    ...
    def _perform_search(self, initial_result):
        ...
        """
        if initial_result.attacked_text.num_words > 700:
        ...
        best_result, search_over = self.get_goal_results([best_attack.attacked_text])
        ...
        for mi in range(self.max_iters):
            ...
            for pop_member in population:
                ...
                similarity_scores.append(float(pop_member.attributes["similarity_score"]))
            ...
            x = similarity_scores
            y = np.exp(x -np.max(x))
            similarity_scores = y / np.sum(y)
            k = np.sum(similarity_scores)
            # if np.sum(similarity_scores)>1.:
            #
            # elif np.sum(similarity_scores)<1.
            # Selection
            ...
            results, search_over = self.get_goal_results(child_texts)
            if search_over:
                return best_result
    ...
    def diff_color(self, original_result, perturbed_result, color_method='ansi', ori_show=1, perturb_show=1):
        ...
        words_1_idxs = []
        t2_equal_idxs = set()
        original_index_map = t1.attack_attrs["original_index_map"]
        ...
        t2 = perturbed_result.replace_words_at_indices(
