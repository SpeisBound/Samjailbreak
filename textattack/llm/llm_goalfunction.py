...
class GoalFunction(ABC):
    ...
    def get_results(self, attacked_text_list, check_skip=False):
        ...
        result consisting of whether the goal has been achieved, the
        ...
        post_data = {'texts': [attacked_text_list[i].text for i in range(len(attacked_text_list))]}
        retries = 0
        max_retries = 10
        while retries < max_retries:
            try:
                model_outputs = requests.post(url='http://127.0.0.1:5001/llm', json={'data': post_data},
                                              verify=False).json()['rst']
                break
        ...
        for attacked_text, raw_output in zip(attacked_text_list, model_outputs):
            if raw_output == 2:
                raw_output = abs(1-self.ground_truth_output)
            if raw_output == -1:
                raw_output = self.ground_truth_output
            displayed_output = self._get_displayed_output(raw_output)
            goal_status = self._get_goal_status(
                raw_output, attacked_text, check_skip=check_skip
            )
            goal_function_score = self._get_score(raw_output, self.ground_truth_output)
            results.append(
    ...
    def _get_goal_status(self, model_output, attacked_text, check_skip=False):
        should_skip = check_skip and self._should_skip(model_output, self.ground_truth_output)
        if should_skip:
            return GoalFunctionResultStatus.SKIPPED
        if self.maximizable:
            return GoalFunctionResultStatus.MAXIMIZING
        if self._is_goal_complete(model_output, self.ground_truth_output):
    ...
    def _should_skip(self, model_output, ground_truth_output):
        if model_output == ground_truth_output:
            return False
        else:
            return True
...
class LLMGoalFunction(GoalFunction):

    """A goal function defined on a model that outputs a probability for some
    number of classes."""

    def __init__(self, query_budget, **kwargs):
